...
public class DragRigidbodyUse : MonoBehaviour {
	...
	private float maxDistanceGrab = 10f; // domyslnie 4f
	...
	void FixedUpdate () {
		if(Input.GetButton(GrabButton)){ //domyslnie (GrabButton)
			if(!isObjectHeld){
				tryPickObject();
		...
		if(Input.GetButton(ThrowButton) && isObjectHeld){ // domyslnie (ThrowButton)
			isObjectHeld = false;
			objectHeld.GetComponent<Rigidbody>().useGravity = true;
			ThrowObject();
		}

		if(Input.GetButton(UseButton)){  //domyslnie (UseButton)
			isObjectHeld = false;
			tryPickObject();
			tryPickupObject = false;
			Use();
	...
	private void tryPickObject(){
		Ray playerAim = playerCam.GetComponent<Camera>().ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f)); //domyslnie (0.5f, 0.5f, 0)
		RaycastHit hit;
	...
	private void holdObject(){
		Ray playerAim = playerCam.GetComponent<Camera>().ViewportPointToRay(new Vector3(0.5f, 0.5f, 0)); // domyslnie 0.5, 0.5, 0

		Vector3 nextPos = playerCam.transform.position + playerAim.direction * distance; //Vector3 nextPos = playerCam.transform.position + playerAim.direction * distance;
		Vector3 currPos = objectHeld.transform.position;
		Debug.DrawRay(transform.position, currPos, Color.green); 

		objectHeld.GetComponent<Rigidbody>().velocity = (nextPos - currPos) * 0.15f; // domyslnie 10
